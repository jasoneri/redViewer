...
if [ ! -f "$shScript" ]; then
    curl -L -o "$shScript" "https://gitee.com/json_eri/redViewer/raw/master/deploy/online_scripts/macos.sh"
    chmod +x "$shScript"
fi
...
invoke_update() {
    ...
    # 更新脚本和版本文件
    sourceScript="$realProjPath/deploy/online_scripts/macos.sh"
